...
function loadNoteToReginMap(nodeToRegionMap, region) {
...
export default class OverlapRegionStrategy {
    ...
    analyzeMain(grid, rows, cols, revealedCells) {
        ...
        regions.forEach((region, key) => {
            ...
            loadNoteToReginMap(nodeToRegionMap, region);
                ...
                for (let j = i + 1; j < regions.length; j++) {
                    ...
                    if (smallerRegion && largerRegion && diffNodes.length > 0) {
                        ...
                        if (newMineCount > 0) {
                            ...
                            // 更新节点到区域的映射
                            loadNoteToReginMap(nodeToRegionMap, newRegion);
                            ...
                            this.log(`新区域: ${diffNodes.map(n => `(${n.row}, ${n.col})`).join(', ')} 雷数: ${newMineCount}`);
                            this.log(`推断说明: 大区域减去小区域后，剩余格子中一定有${newMineCount}个雷`);
                        }
                    }
                }
            }
        ...
        centerToAtLeastRegions.forEach((regions, centerKey) => {
            ...
            regions.forEach((region, index) => {
                loadNoteToReginMap(atLeastNodeToRegionMap, region);
                this.log(`  区域 ${index + 1}:`);
                this.log(`  - 格子: ${region.nodes.map(n => `(${n.row}, ${n.col})`).join(', ')}`);
            ...
            if (nodes.length > mineCount + 1) {
                ...
                for (let i = 0; i < nodes.length - mineCount; i++) {
                    ...
                    if (combination.length === mineCount + 1) {
                        ...
                        loadNoteToReginMap(atMostNodeToRegionMap, atMostRegion);
                        
                        atMostRegions.set(regionKey, atMostRegion);
                        
                        log5(`\n生成的至多区域:`);
                        log5(`- 格子组合: ${combination.map(n => `(${n.row}, ${n.col})`).join(', ')}`);
                        log5(`- 至多雷数: ${atMostRegion.maxMines}`);
                        log5(`- 推断说明: ${mineCount + 1}个格子中至多有${mineCount}个雷，所以至少有一个不是雷`);
                    }
                }
            }
        });

        var log6 = this.buildFlog(true);
        log6('\n=== 节点与至多叠加区域的关系 ===');
        ...
        // 通过至多叠加区域生成新的普通叠加区域
        // 至多叠加区域只能是子集，普通叠加区域减去至多叠加区域，剩余的就是普通叠加区域
        // TODO 代码实现，并打印新的普通叠加区域
        // 通过至多叠加区域生成新的普通叠加区域
        var log7 = this.buildFlog(true);
        log7('\n=== 通过至多叠加区域生成新的至少叠加区域 ===');
        ...
        // 尝试至多区域推断
        const atMostResult = this.analyzeAtMostRegions(atMostRegions);
        if (atMostResult) {
            console.log('\n至多区域推断结束：找到可以确定的无雷格子');
    ...
    // 新增：分析至多区域的方法
    analyzeAtMostRegions(atMostRegions) {
        for (const [_, region] of atMostRegions) {
            const nodes = region.nodes;
            const maxMines = region.maxMines;
            
            // 如果区域中的格子数量比最大雷数多1，说明至少有一个格子不是雷
            if (nodes.length === maxMines + 1) {
                // 检查是否所有其他格子都已被标记为雷
                let unmarkedCount = 0;
                let lastUnmarkedNode = null;
                ...
                // 如果只剩一个未标记的格子，那这个格子一定不是雷
                if (unmarkedCount === 1 && lastUnmarkedNode) {
                    this.log(`\n发现无雷格子：(${lastUnmarkedNode.row}, ${lastUnmarkedNode.col})`);
                    this.log(`原因：该格子所在的组合中已有${maxMines}个格子被标记为雷，而该组合至多只能有${maxMines}个雷`);
                    
                    return {
                        row: lastUnmarkedNode.row,
                        col: lastUnmarkedNode.col,
                        action: 'reveal',
                        isGuess: false
                    };
                }
                ...
                for (const atLeastRegion of atLeastRegions) {
                    ...
                    if (isContained) {
                        ...
                        if (regionCount === atLeastRegion.minMines && diffNodes.length > 0) {
                            this.log(`\n发现有雷推断:`);
                            this.log(`普通区域: ${region.nodes.map(n => `(${n.row}, ${n.col})`).join(', ')} 雷数: ${regionCount}`);
                            this.log(`至少区域: ${atLeastRegion.nodes.map(n => `(${n.row}, ${n.col})`).join(', ')} 最小雷数: ${atLeastRegion.minMines}`);
