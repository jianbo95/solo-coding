<template>
    <div style="height:100%;" class="pageTestMenu" v-if="init">
        <el-container style="height: 100%; overflow: hidden; position: relative;">
            <el-aside class="aside menuAndTagAside2" width="350" style="background:#FFF; border:1px solid #fff; height:100%;">
...
export default {
    ...
    computed: {
        ...
        menusOriginalValue() {
            return JSON.stringify(this.fileListTree, true, 2);
        }
    },
    created() {
    ...
    methods: {
        loadFileList(_call) {
            var url = '/index.php?explorer/treeList&app=editor&type=init';
            $.get(url, {}, (result) => {
                _call(result.data);
            });
        },
