...
var HttpVuePlus = {
    ...
    writeCache: function writeCache(code, url) {
        window.vueCache[url] = code;
    },
    write: function write(code, url) {
    ...
    writeOrigin: function writeOrigin(code, url) {
        this.originWriteMap[url] = code;
    },
    read: function read(url) {
        return this.writeMap[url];
    },
    waitOrigin: function waitOrigin(url, _call) {
        var _this = this;
        ...
        var timer = setInterval(function () {
            ...
            var code = _this.originWriteMap[url];
            if (code != null) {
    ...
    compileScript: function compileScript(code, url) {
     ...
     * @returns 返回加载后的vue文件
     */
    load: function load(code, url, name, resolve) {
        ...
        var newCode = this.loadCompile(code, url, name, function (result) {
            resolve(result);
        });
     ...
     */
    loadCompile: function loadCompile(code, url, name, _call) {
        var lines = code.split('\n');
        var counter = _Counter2.default.auto('loadJs' + url);
        ...
        code = _VueCodeWrap2.default.wrap(code, url, name);
     ...
     * 加载行，语法转换
     */
    loadLine: function loadLine(originLine, url, counter) {
     ...
     * @returns 
     */
    loadImport: function loadImport(line, url, counter) {
        var _this3 = this;

        line = line.trim();
        var info = _UrlParser2.default.parse(url);
        ...
        var path = splits[1].replaceAll('\'', '').replaceAll('"', '').replaceAll(';', '').trim();
        // loadJs
        ...
        if (type == 'js') {
            ...
            var call = counter.callAuto(resourceUrl);

            $.get(resourceUrl, function (code) {
                // console.log('code ' + name + " " + resourceUrl);
                // console.log(code);
                _this3.loadImportSize++;
                ...
                if (code.indexOf('Flag("compileByPublish");') != -1) {
                    ...
                    var jsCode = _this3.loadJs(code, name);
                    _this3.loadScript(jsCode, name, resourceUrl, function () {
    ...
    parseType: function parseType(url) {
        return url.substring(url.lastIndexOf('.') + 1);
    },
    loadExport: function loadExport(line) {
        // 语法转换
        return line.replace('export default', 'module.exports=');
    },
    urlDir: function urlDir(url) {
        return url.substring(0, url.lastIndexOf('/'));
    },
    buildResourceUrl: function buildResourceUrl(urlDir, path) {
        if (path.indexOf('@') == 0) {
            // 根路径直接请求
            return path.replace('@', '.');
        } else {
    ...
    loadJs: function loadJs(js, name) {
        var posStart = js.lastIndexOf('export default');
        var posEnd = posStart + 'export default'.length;
        var code = js.substring(0, posStart) + 'window.' + name + ' = ' + js.substring(posEnd);
    ...
    loadScript: function loadScript(innerHTML, name, resourceUrl, callback) {
