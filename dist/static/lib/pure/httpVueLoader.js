...
var httpVueLoader = function () {
	...
	StyleContext.prototype = {
		...
		withBase: function withBase(callback) {
			...
			if (this.component.baseURI) this.component.getHead().removeChild(tmpBaseElt);
		},
	...
	Component.prototype = {
		...
		compile: function compile() {
			...
			return Promise.all(Array.prototype.concat(this.template.compile(), this.script.compile(), this.styles.map(function (style) {
			...
			beforeCreate: function beforeCreate() {
				...
				for (var componentName in components) {
					...
					if (typeof components[componentName] === 'string' && components[componentName].substr(0, 4) === 'url:') {
						...
						if (isNaN(componentName)) components[componentName] = httpVueLoader.load(comp.url, componentName);else components[componentName] = Vue.component(comp.name, httpVueLoader.load(comp.url, comp.name));
					}
				}
			}
		});
...
})('static/lib/pure/httpVueLoader.js'); exports; 